...
ข้อความที่แอปส่งมามี 2 แบบเท่านั้น แยกกันด้วยหน้าตา:

- **มีวงเล็บ `< >` ครอบ** เช่น `<G,FIST>`, `<B,A,1>`, `<S,1,90>`, `<V,เปิดไฟ>` — ใช้กับ Hand, Gamepad, Servo, Voice แค่ดูวงเล็บก็รู้แล้วว่าข้อความเริ่ม-จบตรงไหน
- **ไม่มีวงเล็บ จบด้วย `;`** เช่น `LED:1;`, `F;` — ใช้กับ Switch I/O, Car, Tilt เหมือนจบประโยคด้วยจุด

อ่านข้อมูลเข้ายังเป็นแบบ Arduino ปกติทุกอย่าง เด็กยังคุม `BT.available()` / `BT.read()` เอง แค่ส่งไบต์ที่อ่านมาให้ `gdd.feed(c)` ทีละตัว ไลบรารีจะดูเองว่าเป็นข้อความแบบไหน แล้วเรียกฟังก์ชันที่ลงทะเบียนไว้ให้ตรงหน้าจอ
